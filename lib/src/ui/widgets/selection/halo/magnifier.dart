...
/// {@endtemplate}
class HaloTextMagnifier extends StatefulWidget {
  ...
  const HaloTextMagnifier({
    super.key,
    required this.magnifierInfo,
  });

  /// A [TextMagnifierConfiguration] that returns a [CupertinoTextMagnifier] on iOS,
  /// [HaloTextMagnifier] on Android, and null on all other platforms, and shows the editing handles
  /// only on iOS.
  static TextMagnifierConfiguration haloAdaptiveMagnifierConfiguration = TextMagnifierConfiguration(
      ...
      magnifierBuilder: (
        ...
        return HaloTextMagnifier(magnifierInfo: magnifierInfo);
      });

  /// The duration that the position is animated if [HaloTextMagnifier] just switched
  /// between lines.
  @visibleForTesting
  static const Duration jumpBetweenLinesAnimationDuration = Duration(milliseconds: 100);

  /// [HaloTextMagnifier] positions itself based on [magnifierInfo].
  ///
  /// {@macro widgets.material.magnifier.positionRules}
  final ValueNotifier<MagnifierInfo> magnifierInfo;

  @override
  State<HaloTextMagnifier> createState() => _HaloTextMagnifierState();
}

class _HaloTextMagnifierState extends State<HaloTextMagnifier> {
  ...
  @override
  void didUpdateWidget(HaloTextMagnifier oldWidget) {
  ...
  void _determineMagnifierPositionAndFocalPoint() {
    ...
    final Offset basicMagnifierOffset = Offset(Magnifier.kDefaultMagnifierSize.width / 2,
    ...
    final double magnifierX = clampDouble(selectionInfo.globalGesturePosition.dx,
        selectionInfo.currentLineBoundaries.left, selectionInfo.currentLineBoundaries.right);
    ...
    final Rect unadjustedMagnifierRect =
        Offset(magnifierX, selectionInfo.caretRect.center.dy) - basicMagnifierOffset & Magnifier.kDefaultMagnifierSize;
    ...
    //     MagnifierController.shiftWithinBounds(bounds: screenRect, rect: unadjustedMagnifierRect);
    final Rect screenBoundsAdjustedMagnifierRect = unadjustedMagnifierRect;
    ...
    final double newRelativeFocalPointX = newGlobalFocalPointX - screenBoundsAdjustedMagnifierRect.center.dx;
    ...
    if (_magnifierPosition != null && finalMagnifierPosition.dy != _magnifierPosition!.dy) {
      ...
      positionShouldBeAnimated = Timer(
          HaloTextMagnifier.jumpBetweenLinesAnimationDuration,
          () => setState(() {
  ...
  Widget build(BuildContext context) {
    assert(_magnifierPosition != null, 'Magnifier position should only be null before the first build.');
    return AnimatedPositioned(
      ...
      duration: _positionShouldBeAnimated ? HaloTextMagnifier.jumpBetweenLinesAnimationDuration : Duration.zero,
      child: Magnifier(
    ...
    //   width: Magnifier.kDefaultMagnifierSize.width,
    //   height: Magnifier.kDefaultMagnifierSize.height,
    //   child:
    // );
  }
}
...
/// widget that is focused on mimicking the behavior of a material magnifier, see [HaloTextMagnifier].
class Magnifier extends StatelessWidget {
  ...
  @visibleForTesting
  static const Size kDefaultMagnifierSize = Size(115, 84);
    ...
    return RawMagnifier(
      decoration: MagnifierDecoration(
        shape: RoundedRectangleBorder(
            borderRadius: borderRadius, side: BorderSide(width: 2, color: HTheme.color.tcTheme)),
        shadows: shadows,
      ),
      magnificationScale: _magnification,
      focalPointOffset:
          additionalFocalPointOffset + Offset(0, kStandardVerticalFocalPointShift + kDefaultMagnifierSize.height / 2),
